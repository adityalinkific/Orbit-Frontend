  ...
  return (
    <div className="p-4 space-y-3">
      {tasks.map((task) => {
        ...
        //  computed INSIDE map
        const deptName = departments[task.department_id] || "UNKNOWN";
        ...
        return (
          ...
          >
            <div className="flex justify-between items-start mb-3">
              ...
              {/*  Department */}
              <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${getDeptStyles(deptName)}`}>
                {deptName}
              </span>
            ...
            <div className="flex justify-between items-center border-t border-gray-50 pt-3">
              ...
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-blue-100 to-purple-100 flex items-center justify-center text-[10px] font-bold text-blue-600 ring-2 ring-white">
                  {String(task.created_by || "UN").slice(0, 2).toUpperCase()}
                </div>
